    ...
    c3_chart_internal_fn.convertUrlToData = function (url, mimeType, keys, done) {
        ...
        $$.d3.xhr(url, function (error, data) {
            ...
            if (type === 'json') {
				...
				//수정: 2015.11.17 IE9에서는 XMLHttpRequest.response가 정의되어 있지 않으므로 responseText로 대체
                d = $$.convertJsonToData(JSON.parse(data.responseText), keys);
            } else if (type === 'tsv') {
                d = $$.convertTsvToData(data.responseText);
            } else {
                d = $$.convertCsvToData(data.responseText);
            }
            done.call($$, d);
        });
    };
